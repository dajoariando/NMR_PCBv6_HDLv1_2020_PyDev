...
def exec_rmt_ssh_cmd_in_datadir ( server_ip, ssh_usr, ssh_passwd, ssh_cmd, data_folder ):
    exec_rmt_ssh_cmd ( server_ip, ssh_usr, ssh_passwd, "cd " + data_folder + " && " + ssh_cmd )


# execute ssh_cmd
def exec_rmt_ssh_cmd ( server_ip, ssh_usr, ssh_passwd, ssh_cmd ):
    # ssh = paramiko.SSHClient()
    # ssh.set_missing_host_key_policy( paramiko.AutoAddPolicy() )
    # ssh.connect( hostname=nmrObj.server_ip , username=nmrObj.ssh_usr, password=nmrObj.ssh_passwd, look_for_keys=False )
    ssh, scp = init_ntwrk ( server_ip, ssh_usr, ssh_passwd )
...
def cp_rmt_file( server_ip, ssh_usr, ssh_passwd, server_path, client_path, filename ):
    # ssh = paramiko.SSHClient()
    # ssh.set_missing_host_key_policy( paramiko.AutoAddPolicy() )
    # ssh.connect( hostname=nmrObj.server_ip , username=nmrObj.ssh_usr, password=nmrObj.ssh_passwd, look_for_keys=False )
...
def cp_rmt_folder( server_ip, ssh_usr, ssh_passwd, server_path, client_path, foldername ):
    # ssh = paramiko.SSHClient()
    # ssh.set_missing_host_key_policy( paramiko.AutoAddPolicy() )
    # ssh.connect( hostname=nmrObj.server_ip , username=nmrObj.ssh_usr, password=nmrObj.ssh_passwd, look_for_keys=False )
